...
VER=3.4
set +x
PIKVMREPO="https://files.pikvm.org/repos/arch/rpi4-aarch64"
...
install-kvmd-pkgs() {
  ...
  version=$(ls "$KVMDCACHE" | \
    grep -oP "$platform-\K(\d+\.\d+)(?=-\d+-any\.pkg\.tar\.xz)" | \
    sort -V | tail -n 1)

# uncompress platform package first
  i=$( ls ${KVMDCACHE}/${platform}-${version}*.tar.xz )  ### install the most up to date kvmd-platform package
...
# then uncompress, kvmd-{version}, kvmd-webterm, and janus packages
  for i in $( ls ${KVMDCACHE}/*.tar.xz | egrep 'kvmd-${version}|webterm' )
  do
    case $i in
